#!/usr/bin/env python3
"""
Umfassende Tests des Patient-Management-Systems - Finale Version
...
class PatientManagerTestSuite:
    ...
    def create_test_patient(self, patient_type: str, **overrides) -> Patient:
        ...
        Args:
            patient_type: Typ des Test-Patienten (basic, val, dup, etc.)
            **overrides: Überschreibungen der Standardwerte
            
        Returns:
            Patient-Objekt
        """
        # Mapping für verschiedene Patiententypen
        patient_names = {
            'basic': ('Testmann', 'Adam'),
            'val_empty': ('Testmann', ''),  # Für Validierungstest
            'val_invalid': ('Testmann', 'Valid'),
            'dup_first': ('DupTest', 'First'),
            'dup_second': ('DupTest', 'First'),
            'upd': ('Testmann', 'Eva'),
            'search_male': ('Searchmann', 'Max'),
            'search_female': ('Searchfrau', 'Anna'),
            'search_male2': ('Suchmann', 'Peter'),
            'del': ('Deletmann', 'Karl'),
            'edge_extreme': ('Extremmann', 'Walter'),
            'edge_long': ('Musterfrau-Schmidt', 'Gabriele-Maria'),
            'edge_future': ('Futuremann', 'Max'),
            'perf_create': ('Perfmann', 'Quick'),
            'perf_update': ('Updatemann', 'Felix'),
            'mass_female': ('Massfrau', 'Lisa'),
            'mass_male': ('Massmann', 'Tom')
        }
        
        # Standardwerte
        lastname, firstname = patient_names.get(patient_type, ('Testmann', 'Anna'))
        ...
        defaults = {
            ...
            'folder_slug': None  # Wird automatisch generiert
        }
        ...
        # Patient - folder_slug wird automatisch aus Demographics generiert
        patient = Patient(
            consents=consents,
            demographics=demographics,
            surgery=surgery,
            notes=defaults.get('notes', f'Test-Patient {patient_type}')
        )
    ...
    def test_create_patient_validation_errors(self) -> Dict[str, Any]:
        ...
        try:
            ...
            test_cases = [
                ...
                {'patient_type': 'val_empty', 'expected_error': 'Name darf nicht leer sein'},
                # Ungültige Daten
                {'patient_type': 'val_invalid', 'dob': date.today(), 'expected_error': 'Geburtsdatum muss in der Vergangenheit liegen'},
                # Ungültige Messwerte
                {'patient_type': 'val_invalid', 'nose_length_mm': 100, 'expected_error': 'außerhalb des Normalbereichs'},
                # Fehlende Einwilligungen
                {'patient_type': 'val_invalid', 'photo_consent': False, 'expected_error': 'Keine Foto-Einwilligung'},
            ]
            
            results = []
            for i, case in enumerate(test_cases):
                patient_type = case.pop('patient_type')
                patient = self.create_test_patient(patient_type, **case)
    ...
    def test_update_patient(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # Patient erstellen
            patient = self.create_test_patient("upd", satisfaction_vas=5)
            success, message, patient_id = patient_manager.create_patient(patient)
    ...
    def test_search_patients(self) -> Dict[str, Any]:
        ...
        try:
            ...
            test_patients = [
                self.create_test_patient("search_male", gender=Gender.MALE, technique="Offen"),
                self.create_test_patient("search_female", gender=Gender.FEMALE, technique="Geschlossen"),
                self.create_test_patient("search_male2", gender=Gender.MALE, technique="Geschlossen"),
            ]
    ...
    def test_delete_patient(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # Patient erstellen
            patient = self.create_test_patient("del")
            success, _, patient_id = patient_manager.create_patient(patient)
    ...
    def test_edge_cases(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # Test 3: Operation in der Zukunft (sollte fehlschlagen)
            try:
    ...
    def test_performance_crud_operations(self) -> Dict[str, Any]:
        ...
        try:
            ...
            create_times = []
            for i in range(3):
                patient = self.create_test_patient("perf_create")
                start_time = time.time()
    ...
    def test_mass_operations(self) -> Dict[str, Any]:
        ...
        try:
            ...
            created_patients = []
            for i in range(5):
                patient_type = "mass_female" if i % 2 == 0 else "mass_male"
