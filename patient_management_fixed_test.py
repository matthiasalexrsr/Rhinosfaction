#!/usr/bin/env python3
"""
Umfassende Tests des Patient-Management-Systems - Korrigierte Version
...
class PatientManagerTestSuite:
    ...
    def create_test_patient(self, patient_id: str, **overrides) -> Patient:
        ...
        # Standardwerte - nur gültige Namen ohne Zahlen
        defaults = {
            'lastname': 'Test',
            'firstname': f'Patient{self.patient_counter}',  # Nur gültige Zeichen
            'gender': Gender.MALE,
            'dob': date(1990, 1, 1),
        ...
        # Patient - folder_slug wird automatisch generiert
        patient = Patient(
            consents=consents,
            demographics=demographics,
            surgery=surgery,
            notes=defaults.get('notes', f'Test-Patient {patient_id}'),
            folder_slug=defaults['folder_slug']
        )
    ...
    def test_create_patient_validation_errors(self) -> Dict[str, Any]:
        ...
        try:
            ...
            test_cases = [
                ...
                # Ungültige Daten
                {'lastname': 'Test', 'firstname': 'Invalid', 'dob': date.today(), 'expected_error': 'Geburtsdatum muss in der Vergangenheit liegen'},
                # Ungültige Messwerte
                {'lastname': 'Test', 'firstname': 'Invalid', 'nose_length_mm': 100, 'expected_error': 'außerhalb des Normalbereichs'},
                # Fehlende Einwilligungen
                {'lastname': 'Test', 'firstname': 'Invalid', 'photo_consent': False, 'expected_error': 'Keine Foto-Einwilligung'},
            ]
    ...
    def test_search_patients(self) -> Dict[str, Any]:
        ...
        try:
            ...
            test_patients = [
                self.create_test_patient("search001", gender=Gender.MALE, technique="Offen"),
                self.create_test_patient("search002", gender=Gender.FEMALE, technique="Geschlossen"),
                self.create_test_patient("search003", gender=Gender.MALE, technique="Geschlossen"),
            ]
    ...
    def test_delete_patient(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # Patient erstellen
            patient = self.create_test_patient("del001")
            success, _, patient_id = patient_manager.create_patient(patient)
        ...
        try:
            ...
            try:
                patient_long_name = self.create_test_patient(
                    "edge002",
                    lastname="Musterfrau Schmidt",  # Langer Name
    ...
    def test_performance_crud_operations(self) -> Dict[str, Any]:
        ...
        try:
            ...
            create_times = []
            for i in range(3):
                patient = self.create_test_patient(f"perfcreate{i}")
                start_time = time.time()
